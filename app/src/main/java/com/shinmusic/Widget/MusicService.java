...
public class MusicService extends Service implements MusicPlayerCompletionListener {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        try {
            if (action.equals(WidgetProvider.ACTION_PLAY_PAUSE)) {
                ...
                BaiHat baihat = (BaiHat) intent.getExtras().get("song");
                jumpTo(baihat);
            }
    ...
    private void playMusic() throws IOException {
        Log.d(TAG, "PLAY");
        BaiHat baiHat = MusicLoader.getInstance(this).getCurrent();

        if (player.isPaused()) {
            player.play();
        } else {
            player.setSong(baiHat);
            player.play();
        }

        updateUI(baiHat.getTenBaiHat(), baiHat.getIdBaiHat(), baiHat.getLuotThich(), true);
        Log.i("Music Service", "Playing: " + baiHat.getTenBaiHat());
    }
    ...
    private void pauseMusic() {
        ...
        if (player.isPlaying()) {
            BaiHat baiHat = MusicLoader.getInstance(this).getCurrent();
            updateUI(baiHat.getTenBaiHat(), baiHat.getIdBaiHat(), baiHat.getCaSy(), false);
    ...
    private void nextSong() throws IOException {
        if (player != null) {
            BaiHat nextSong = MusicLoader.getInstance(this).getNext();
            player.setSong(nextSong);

            updateUI(nextSong.getTenBaiHat(), nextSong.getCaSy(), nextSong.getLuotThich(), player.isPlaying());
        }
    }
    ...
    private void previousSong() throws IOException {
        ...
        if (player != null) {
            BaiHat prevSong = MusicLoader.getInstance(this).getPrevious();
            player.setSong(prevSong);

            updateUI(prevSong.getTenBaiHat(), prevSong.getCaSy(), prevSong.getLuotThich(), player.isPlaying());
        }
    }

    private void jumpTo(BaiHat song) throws IOException {
    ...
    public void onMusicCompletion() throws IOException {
        nextSong();
        player.play();
        BaiHat song = MusicLoader.getInstance(this).getCurrent();
        updateUI(song.getTenBaiHat(), song.getCaSy(), song.getLuotThich(), true);
    }
